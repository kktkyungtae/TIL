...
V, E = map(int, input().split()) # V는 정점수 E : ㄱㅏㄴ선수
...
```python
## 재귀 호출  > 함수호출 트리 (깊이 우선 트리)
# 스택을 통해 방문하지 않는다.. 함수 호출에 자연스럽게 관리가 되니까. . 코드가 간결해짐.
def DFS(v):
    visited = True
    print(v,end='')
    for w in G[v]:
       ...
       # 방문하지 않은 인접정점을 찾고 나서,, 스택에 저장하고 함수 호출하고 ... 함수는 정지하고 함수 호출하면서 계속돈다.
    # 


V, E = map(int, input().split()) # V는 정점수 E : ㄱㅏㄴ선수
G = [[] for _ in range(V+1)]
for i in range(E): # 방문안한 정점 찾기 
    U,V = map(int, int().split())
    G[u].append(v)
    G[v].append(u)
