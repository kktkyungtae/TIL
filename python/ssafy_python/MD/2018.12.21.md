...
텔레그램 웹에서 msg를 텔레그램 서버에 보내면 잠시 가지고 있다.

c9에서 텔레그램서버 에 겟업데이츠 해주면 msg가 텔레그램에서 c9로 보내.



2

getUpdates 대신에 webhook을 했다?

웹훅은 뭐냐면.,, url을 등록한다. 토큰으로 받아서 등록한다.
...
주소창에 djsajdasdlk입력하는게 get이다.

get해야 데이터? web을 띄워줌

request.get -> get 방식으로 요청을 한다.
...
POST ((request Method에))

파라미터 정보가 숨어 있다..URL에 없고
...
마스터키? 함수를 썼는데 맞춰서 만들었다.

공통적인 코드만 있어도 조인해서 원하는 메세지 형식으로 바꿀 수 있었다.

크롤링 코드에서 셀렉트해서 ㅇㄴ마ㅣ어ㅏㅣ너이ㅓㅁㄴ하고 위에는 언마ㅣㅓ이ㅏㄴ머이ㅏ머했다..

매계변수 : 매개변수를 ()안에 넣어주면 밑에서 함수로 쓸 수 있다?



10

오늘 날짜가 매일 들어가서 되게는 우쨰? 타임!! 사용
